...
#             print( n_t[task][k])
            mean = np.reshape(M[task][k], (p,1))
#             for j in range(n_t[task][k]):
                # on crée n_j[task][k] vecteurs aléatoires de taille 1xp
            X_k=mean@np.ones((1,n_t[task][k]))+np.random.randn(p,n_t[task][k])
...
def merging_center(MM, diag, t, m, p, n, n_t, task_target=None, display=False, normalization=False):
    ...
    for i in range(len(MM)):
        for l in range(m):
            ...
            # emp_means1.append(np.reshape(MM1[i].T[l], (p, 1)))
    ...
    if display:
        print("y : ")
        matprint(y)
    # xy_product = X.dot(J).dot(y)
    # xy_product=xy_product/np.linalg.norm(xy_product)
